    ...
    try:
        ...
        for iface in ifaces:
            ...
            try:
                ...
                for net2,net1 in zip(networks2,networks):
                    SSID = (net2.dot11Ssid.SSID[:net2.dot11Ssid.SSIDLength]).decode('utf-8')
                    BSSID = ':'.join('%02x' % b for b in net2.dot11Bssid).upper()
                    signal_strength = str(net2.lRssi)
                    CenterFreq = net2.ulChCenterFrequency/1000000 # khz    
                    typ = net2.dot11BssPhyType
                    Algo = net1.dot11DefaultAuthAlgorithm
                    rate = net2.wlanRateSet.uRateSetLength
                   # for i in range(0,126):
                    #    rate_in_mbps = (rate[i] & 0x7FFF) * 0.5
                    typ2=""
                    if typ == 7:
                        typ2 = "802.11n"
                    elif typ ==8:
                    ...
                    print (" Rssi:" + signal_strength +' dbm ',
                           " Freq:",CenterFreq ,'GHz',
                           " type:",typ2,
                           " algo:",Algo,
                           " rate:",rate,
                    ...
                    #BSSI_Values[BSSID]= [SSID,signal_strength,q,CenterFreq]
